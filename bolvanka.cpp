...
int main()
{
    for (int i = 2; i < 17; i++)
    {
        ...
        cl_program program = build_program(context, deviceID, CONVOLUTION_PROGRAM1);
        ...
        generateGaussianKernel(kernelValues, kernelSize, 3.0f);
        //generateBoxBlurKernel(kernelValues, kernelSize);
        ...
        cl_event event;
        size_t local_work_size[] = { 16, 16 }; // Размер рабочей группы
        errcode_ret = clEnqueueNDRangeKernel(queue, kernel, 2, NULL, global_work_size, local_work_size, 0, NULL, &event);
        ...
        clWaitForEvents(1, &event); // Ожидаем завершения конкретного события
